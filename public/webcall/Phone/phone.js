...
let SipUsername = user.id; // eg: webrtc
let SipPassword = user.xid; // eg: webrtc
...
let hostingPrefex = user.hostingPrefex + "/"; // Use if hosting off root directiory. eg: "/phone/" or "/static/"
...
let CallWaitingEnabled = false; // Rejects any inbound call if you are on a call already.
...
// let EnableOutgoing = false; // Enables Email sending to the server (requires onward services)
...
function InitUi() {
    ...
    var phone = $("#Phone");
    phone.empty();
    phone.attr("class", "pageContainer col-md-12 ");

    // Left Section
    var leftSection = $("<div/>");
    leftSection.attr("id", "leftContent");
    leftSection.attr("style", "float:left; height: 1280px; width:100%");

    var leftHTML = "<table style=\"height:100%; width:100%\" cellspacing=5 cellpadding=0>";
    leftHTML += "<tr><td class=streamSection style=\"height: 92px\">";
    ...
    // Line
    leftHTML += "<div style=\"margin-left:5px; margin-right:5px; margin-bottom: 5px; border-top:1px solid #cbcbcb\"></div>";
    ...
    rightSection.attr("style", "margin-left: 320px; height: 100vh");
...
function AddCallMessage(buddy, session) {
    ...
    $.post(user.end_url, {
        session_id: newMessageJson.SessionId,
        duration: newMessageJson.Billsec,
        terminate: newMessageJson.Terminate,
        reason_text: newMessageJson.ReasonText,
...
function ShowDial() {
    ...
    html += "<button class=\"dialButtons\" id=dialAudio style=\"background-color: #3fbd3f;color: white;\" title=\"" + lang.audio_call + "\" onclick=\"DialByLine('audio')\"><i class=\"fa fa-phone\"></i></button>";
...
function AddLineHtml(lineObj) {
    ...
    // Calling UI --------------------------------------------------------------------------
    html += "<div id=\"line-" + lineObj.LineNumber + "-calling\" style='margin-top: 13px;' >";
...
function ShowMyProfile() {
    ...
    AudioVideoHtml += "<div style=\"text-align:center; margin-top:10px;display: none;\"><video id=local-video-preview class=previewVideo muted playsinline></video></div>";
...
function getPicture(buddy, typestr, ignoreCache) {
    if (buddy == "profilePicture") {
        return user.base_url + '/hyper/assets/images/users/avatar-1.jpg'
