                            ...
                            <form method="POST" action="" class="form-horizontal" id="createCampaign">
                                ...
                                <div class="form-group row mb-2">
                                    ...
                                    <div class="col-lg-10" id="agentselect">
                                        <select class="select2 select2-multiple" multiple="multiple" id="agent" name="agent[]" data-placeholder="Choose Agent ...">
                                            @foreach ($employee as $item)
                                                <option value="{{ $item->id }}">{{ $item->user->name }} {{ $item->user_id == Auth::user()->id ? '(YOU)' : '' }} </option>
                                            @endforeach
...
<script>
        $('#save-form').click(function(event) {
            ...
            $.easyAjax({
                url: "{{ route('member.campaigns.store') }}",
