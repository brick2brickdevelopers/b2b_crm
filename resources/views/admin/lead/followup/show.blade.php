                ...
                <div class="sttabs tabs-style-line">
                    <div class="white-box">
                        ...
                        <nav>
                            <ul>
                                <li><a
                                        href="{{ route('admin.leads.show', $lead->id) }}"><span>@lang('modules.lead.profile')</span></a>
                                </li>
                                <li><a
                                        href="{{ route('admin.proposals.show', $lead->id) }}"><span>@lang('modules.lead.proposal')</span></a>
                                </li>
                                <li><a
                                        href="{{ route('admin.lead-files.show', $lead->id) }}"><span>@lang('modules.lead.file')</span></a>
                                </li>

                                <li class="tab-current"><a
                                        href="{{ route('admin.leads.followup', $lead->id) }}"><span>@lang('modules.lead.followUp')</span></a>
                                </li>
                                @if ($gdpr->enable_gdpr)
                                    <li><a
                                            href="{{ route('admin.leads.gdpr', $lead->id) }}"><span>@lang('app.menu.gdpr')</span></a>
                                    </li>
                                @endif
                            </ul>
                        </nav>
                                ...
                                <div class="col-xs-12" id="follow-list-panel">
                                    <div class="white-box">
                                        ...
                                        <div class="row m-b-10">
                                            <div class="col-md-5">
                                                @if ($lead->next_follow_up == 'yes')
                                                    <a href="javascript:;" id="show-new-follow-panel"
                                                        class="btn btn-success btn-outline"><i class="fa fa-plus"></i>
                                                        @lang('modules.followup.newFollowUp')</a>
                                                @endif
                                            </div>
                                            <div class="col-md-3">
                                            ...
                                            <div class="col-md-4">
                                                <select class="selectpicker" data-style="form-control" id="sort-task"
                                        ...
                                        <div class="list-group" id="listGroup"
                                            ...
                                            @forelse($lead->follow as $follow)
                                                @php
                                                    $followup = App\LeadFollowUp::findOrFail($follow->id)->withCustomFields();
                                                    $fields = $followup->getCustomFieldGroupsWithFields()->fields;
                                                    
                                                @endphp
                                                <a href="javascript:;" data-follow-id="{{ $follow->id }}"
                                                    id="followUpBox{{ $follow->id }}" class="list-group-item edit-task">
                                                    <h4 class="list-group-item-heading sbold">@lang('app.createdOn'):
                                                        {{ $follow->created_at->format($global->date_format . ' ' . $global->time_format) }}
                                                        <button type="button" title="@lang('app.delete')"
                                                            onclick="removeFollow({{ $follow->id }});"
                                                            class="btn btn-danger float-right followup-remove"><i
                                                        ...
                                                        <div class="row">
                                                            ...
                                                            @foreach ($fields as $field)
                                                                <div class="col-md-6">
                                                                    ...
                                                                    <p class="text-muted">
                                                                        @if ($field->type == 'text')
                                                                            {{ $followup->custom_fields_data['field_' . $field->id] ?? '' }}
                                                                        ...
                                                                        @elseif($field->type == 'radio')
                                                                            {{ !is_null($followup->custom_fields_data['field_' . $field->id]) ? $followup->custom_fields_data['field_' . $field->id] : '-' }}
                                                                        @elseif($field->type == 'select')
                                                                            {{ !is_null($followup->custom_fields_data['field_' . $field->id]) && $followup->custom_fields_data['field_' . $field->id] != '' ? $field->values[$followup->custom_fields_data['field_' . $field->id]] : '-' }}
                                                                        @elseif($field->type == 'checkbox')
                                                                            <ul>
                                                                                @foreach ($field->values as $key => $value)
                                                                                    @if ($followup->custom_fields_data['field_' . $field->id] != '' && in_array($value, explode(', ', $followup->custom_fields_data['field_' . $field->id])))
                                                                                        <li>{{ $value }}</li>
                                                                                    @endif
                                                                                @endforeach
                                                                            </ul>
                                                                        @elseif($field->type == 'date')
                                                                            {{ !is_null($followup->custom_fields_data['field_' . $field->id]) ? \Carbon\Carbon::parse($followup->custom_fields_data['field_' . $field->id])->format($global->date_format) : '--' }}
                                                                        @endif
                                                                    </p>
                                    ...
                                    <div class="panel panel-default">
                                        <div class="panel-heading "><i class="ti-plus"></i> @lang('modules.followup.newFollowUp')
                                            <div class="panel-action">
                                                <a href="javascript:;" id="hide-new-follow-panel"><i
                                                        class="ti-close"></i></a>
                                            </div>
                                        </div>
                                                        ...
                                                        <div class="col-xs-12">
                                                            <div class="form-group">
                                                                ...
                                                                <input type="text" name="next_follow_up_date"
                                                                    autocomplete="off" id="next_follow_up_date"
                                                                    class="form-control">
                                                                <input type="hidden" name="type"
                                                                    class="form-control datepicker" value="datetime">
